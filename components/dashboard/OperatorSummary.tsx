  ...
  return (
    <div className="rounded-[8px] border border-[#1E2235] bg-[#131625] p-4">
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        {suffix && <span className="font-display text-sm text-ok">{suffix}</span>}
      </div>
  ...
  return (
    <div className="h-full min-h-0 space-y-4 overflow-y-auto">
      <div className="grid grid-cols-4 gap-4">
        <KpiCard label="Tasa de excepción" value="8.3%" meta="<10%" tone="ok" suffix="↓" />
        <KpiCard label="Ejecución autónoma" value="91.7%" meta=">90%" tone="ok" />
      ...
      {circuitOpenCount > 0 && (
        <div className="flex items-center gap-3 rounded-data border border-warn/40 bg-warn/15 p-4 text-warn">
          <AlertTriangle className="h-5 w-5" />
          <p className="text-sm">Hay {circuitOpenCount} cortacircuito activo. Las decisiones asociadas permanecen congeladas hasta revisión del operador.</p>
        </div>
      )}
    </div>
  );
