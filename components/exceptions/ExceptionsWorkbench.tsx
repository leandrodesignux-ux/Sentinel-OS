...
const kindStyles: Record<ExceptionKind, { border: string; dot: string; badge: string }> = {
  critical: { border: "border-l-critical", dot: "bg-critical", badge: "border-critical/40 bg-critical/15 text-red-600" },
  legal: { border: "border-l-orange-500", dot: "bg-orange-500", badge: "border-orange-500/40 bg-orange-500/15 text-orange-700" },
  cascade: { border: "border-l-purple-500", dot: "bg-purple-500", badge: "border-purple-500/40 bg-purple-500/15 text-purple-700" },
  uncertainty: { border: "border-l-warn", dot: "bg-warn", badge: "border-warn/40 bg-warn/15 text-yellow-700" },
  operational: { border: "border-l-warn", dot: "bg-warn", badge: "border-warn/40 bg-warn/15 text-yellow-700" },
};
  ...
  return (
    <div className="rounded-data border border-primary/30 bg-primary/10 p-4">
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="inline-flex rounded-badge border border-primary/40 px-2 py-1 font-display text-[10px] text-[var(--status-accent)]">{agents.length} agentes · mismo error</div>
          <p className="mt-2 font-accent text-lg">Grupo de excepciones {getExceptionLabel(kind).toLowerCase()}</p>
          <p className="text-sm text-[var(--text-secondary)]">Impacto agregado: <span className="text-red-600">${Math.round(agents.reduce((sum, agent) => sum + agent.economic_risk.amount, 0) / 1000)}K</span></p>
        </div>
        <Dialog>
          <DialogTrigger asChild>
            <button className="rounded-badge border border-primary/50 bg-primary/15 px-4 py-2 font-display text-xs text-[var(--status-accent)]">Resolver todos con una acción</button>
          </DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-4">
              <div className="rounded-data border bg-background/50 p-3">
                <p className="font-display text-xs text-[var(--text-muted)]">Agentes afectados</p>
                <p className="mt-2 text-sm text-foreground/75">{agents.map((agent) => agent.id).join(", ")}</p>
              </div>
              <div className="rounded-data border bg-background/50 p-3">
                <p className="font-display text-xs text-[var(--text-muted)]">Acción aplicada</p>
              ...
              <button disabled={!canConfirm} onClick={confirmResolution} className="rounded-badge border border-green-200 bg-green-50 px-4 py-2 font-display text-xs text-green-700 disabled:cursor-not-allowed disabled:opacity-40">Confirmar resolución</button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      {toastCountdown !== null && (
        <div className="mt-3 flex items-center gap-2 rounded-data border border-green-200 bg-green-50 p-2 text-sm text-green-700">
...
function ExceptionWorkCard({ agent, resolved, onResolve, onReject, onAudit }: { agent: Agent; resolved: boolean; onResolve: () => void; onReject: () => void; onAudit: () => void }) {
  ...
  return (
    <article className={cn("rounded-data border border-[var(--bg-border)] border-l-4 bg-white p-4 transition", styles.border, resolved && "opacity-45")}> 
      <div className="flex items-center gap-3">
        <span className={cn("h-2.5 w-2.5 animate-status-pulse rounded-full", styles.dot)} />
        <p className="font-display text-sm text-[var(--text-primary)]">{agent.id} · {agent.name}</p>
        <span className={cn("rounded-badge border px-2 py-1 font-display text-[10px]", styles.badge)}>{getExceptionLabel(kind)}</span>
        <span className="ml-auto inline-flex items-center gap-1 font-display text-xs text-[var(--text-muted)]"><Clock3 className="h-3.5 w-3.5" /> {timeAgo(agent)}</span>
        <span className="font-display text-sm text-red-600">${economicImpactK(agent)}K</span>
      </div>
      <p className="mt-3 truncate text-sm text-[var(--text-secondary)]">{exceptionDescription(agent)}</p>
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <button onClick={onResolve} className="rounded-badge border border-green-200 bg-green-50 px-3 py-2 font-display text-xs text-green-700"><CheckCircle2 className="mr-1 inline h-3.5 w-3.5" /> Aprobar</button>
        <button onClick={onReject} className="rounded-badge border border-critical/40 bg-critical/10 px-3 py-2 font-display text-xs text-red-600"><XCircle className="mr-1 inline h-3.5 w-3.5" /> Rechazar</button>
        <button onClick={onAudit} className="rounded-badge border border-primary/40 bg-primary/10 px-3 py-2 font-display text-xs text-[var(--status-accent)]"><FileSearch className="mr-1 inline h-3.5 w-3.5" /> Ver auditoría →</button>
        {canFreeze && <button className="rounded-badge border border-warn/40 bg-warn/10 px-3 py-2 font-display text-xs text-yellow-700"><ShieldX className="mr-1 inline h-3.5 w-3.5" /> Congelar agente</button>}
      </div>
    </article>
  );
}
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <header className="flex items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          <label className="flex items-center gap-2 font-display text-xs text-[var(--text-secondary)]">
            Ordenar por:
            <select value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)} className="rounded-badge border border-[var(--bg-border)] bg-white px-3 py-2 text-[var(--text-primary)]">
          ...
          <button onClick={() => setShowResolved((value) => !value)} className={cn("rounded-badge border border-[var(--bg-border)] px-3 py-2 font-display text-xs", showResolved ? "bg-primary/15 text-[var(--status-accent)]" : "bg-white text-[var(--text-secondary)]")}>Mostrar resueltas</button>
        </div>
      </header>

      <section className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
        {visibleExceptions.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center rounded-data border border-[var(--bg-border)] bg-white text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full border border-ok/30 bg-ok/10"><CheckCircle2 className="h-8 w-8 text-green-700" /></div>
            <h3 className="mt-4 font-accent text-xl">Flota operando en piloto automático</h3>
            <p className="mt-2 text-sm text-[var(--text-muted)]">Última excepción registrada hace 12 minutos.</p>
          </div>
        ) : (
          <>
