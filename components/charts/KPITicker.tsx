  ...
  return (
    <div className="grid grid-cols-2 gap-2 font-display text-xs md:grid-cols-3 xl:grid-cols-6">
      {emergencyHalt.active && (
        <button onClick={reactivateFleet} className="rounded-data border border-critical/50 bg-critical/15 p-2 text-left text-critical shadow-danger">
          <span className="text-foreground/45">Fleet Status</span>
          <p className="animate-signal-blink">EMERGENCY HALT</p>
          <span className="text-[10px] text-foreground/45">Reactivar {secondsLeft}s</span>
        </button>
      )}
