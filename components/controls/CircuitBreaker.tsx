  ...
  return (
    <div className="rounded-data border bg-background/60 p-3">
      <div className="mb-3 flex items-center justify-between">
        <p className="font-accent text-sm text-foreground">Circuit breaker</p>
        <button onClick={() => setCircuitBreakerLevel(agentId, 0)} className="rounded-badge border border-ok/40 px-2 py-1 font-display text-[10px] text-ok">RESET</button>
      </div>
      <div className="grid gap-2">
        {levels.map((item) => (
          <button key={item.level} onClick={() => setCircuitBreakerLevel(agentId, item.level)} className={`rounded-badge border px-2 py-2 text-left transition ${currentLevel === item.level ? "border-critical bg-critical/15 text-critical" : "border-border bg-card/60 text-foreground/65"}`}>
