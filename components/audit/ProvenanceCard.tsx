  ...
  return (
    <div className="rounded-data border bg-background/60 p-3 font-display text-xs">
      ...
      <div className="mt-2 grid grid-cols-[120px_1fr] gap-2">
        <span>Qué pasó</span><span className="text-primary">{step?.action ?? agent.current_task.description}</span>
        <span>Con qué datos</span><span className="text-primary">{step ? `${step.data_source} · ${step.input_tokens} tokens` : `${agent.metadata.region} · ${agent.metadata.tokens_used} tokens`}</span>
        <span>Modelo</span><span className="text-primary">{agent.metadata.model_version}</span>
        <span>Policy</span><span className="text-primary">{agent.metadata.prompt_policy}</span>
        <span>Cuándo</span><span className="text-primary">{step ? new Date(step.timestamp).toLocaleTimeString() : agent.metadata.last_human_touch}</span>
        <span>Riesgo aportado</span><span className="text-primary">{step?.risk_contribution ?? agent.risk_level}</span>
      </div>
      <div className="mt-3">
